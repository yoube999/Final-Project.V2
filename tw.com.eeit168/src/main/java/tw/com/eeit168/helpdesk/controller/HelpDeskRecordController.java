...
@RestController
@CrossOrigin
@RequestMapping("/project/eeit168/helpdeskrecord")
...
public class HelpDeskRecordController {
	...
	public String selectRecordByStatus(@RequestBody String json) {
		...
		// 回傳給前端查詢資料總數做為分頁依據
		long count = helpDeskRecordService.recordsTotal(json);
		...
		if (helpDeskRecords != null && !helpDeskRecords.isEmpty()) {
			for (HelpDeskRecordBean helpDeskRecord : helpDeskRecords) {
				...
				// 將迴圈得到的資料放入array陣列中
				JSONObject item = new JSONObject().put("record_id", helpDeskRecord.getRecord_id())
						.put("username", helpDeskRecord.getUsername())
						.put("return_title", helpDeskRecord.getReturn_title())
						.put("member_profile_id", helpDeskRecord.getMember_profile_id()).put("return_date", returnDate);
	...
	public String selectUserById(@PathVariable(name = "member_profile_id") Integer member_profile_id) {
		...
		if (member != null) {
			...
			// 將得到的資料放入array陣列中
			JSONObject item = new JSONObject().put("username", member.getUsername()).put("birthday", birthday)
					.put("gender", member.getGender()).put("phone_number", member.getPhone_number())
	...
	public String selectRecordById(@PathVariable(name = "record_id") Integer record_id) {
		JSONObject responseJson = new JSONObject();
		JSONArray array = new JSONArray();
		HelpDeskRecordBean record = helpDeskRecordService.selectRecordById(record_id);
		if (record != null) {
			...
			// 將得到的資料放入array陣列中
			JSONObject item = new JSONObject().put("username", record.getUsername()).put("user_account", record.getUser_account())
					.put("return_title", record.getReturn_title()).put("record_id", record.getRecord_id())
					.put("return_description", record.getReturnDescription()).put("return_date", returnDate);
